        ...
        virtual void operator ()(const Range &r) const CV_OVERRIDE
        {
            ...
            for (size_t row = startIndex / width; index < endIndex; row++)
            {
                ...
                for (int d = last - 1; d >= 0; d--)
                {
                    ...
                    const size_t inner = row_offsets.size();
                    row_offsets.resize(inner * count);
                    // Fill backwards to keep the original first block available.
                    for (size_t k = count; k-- > 0;)
                        for (size_t i = 0; i < inner; i++)
                            row_offsets[k * inner + i] = row_offsets[i] + tap_offsets[d][begin + k];
                }
                ...
                auto sum_four = [&](size_t x, size_t input_offset) {
                    ...
                    for (size_t offset : row_offsets)
                        for (size_t k = x_starts[x]; k < x_starts[x + 1]; k++)
                            val = v_add(val, v_load(row_data + offset + x_offsets[k] + input_offset));
                    return v_add(val, vbias);
                };
#endif
                size_t x = index - row_start;
                while (index < row_end)
                {
                    ...
                    for (size_t offset : row_offsets)
                        for (size_t k = x_starts[x]; k < x_starts[x + 1]; k++)
                            val += row_data[offset + x_offsets[k]];
                    data_im_[index] = val + bias;
                    x++;
                    index++;
                }
